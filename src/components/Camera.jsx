...
export default function CameraRig({ avoidRightPanel = false }) {
  ...
  const PANEL_FOCUS_OFFSET_X = avoidRightPanel ? 1.8 : 0;

  useFrame(() => {
    // 1. Find where the player is heading in isometric space
    const playerWorldPos = toIso(gridX, gridZ);
    const framedTargetX = playerWorldPos.x + PANEL_FOCUS_OFFSET_X;

    // 2. Smoothly track the camera focus point
    lookAtTarget.current.x = THREE.MathUtils.lerp(lookAtTarget.current.x, framedTargetX, 0.08);
    ...
    camera.position.x = THREE.MathUtils.lerp(camera.position.x, framedTargetX + CAMERA_OFFSET_X, 0.08);
